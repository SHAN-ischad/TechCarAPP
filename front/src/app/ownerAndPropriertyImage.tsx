...
export default function OwnerAndPropriertyImage() {
    ...
    const { cadastrarUsuario } = useCadasterUser();
    ...
    const handleContinue = async () => {
        ...
        // Atualize o contexto com os dados retornados (incluindo a imagem)
        if (response?.data?.usuario) {
            setCadastro(response.data.usuario);
        }
        router.replace({ pathname: '/loadingScreen', params: { tipoConta: 'proprietário' } });
    };
    ...
    return (
        <ScrollView className='flex-1 bg-white'>
            ...
            <View className='w-full min-h-[500px] items-center justify-center  gap-10'>
                <View className='flex-row gap-[10px] max-[900px]:flex-col justify-center'>
